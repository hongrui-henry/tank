class Game {
        ...
        window.addEventListener('keydown', (e) => {
            if (e.code === 'Escape') {
                ...
                if (this.gameState === 'PLAYING' && !this.isPaused) {
                    ...
                    if (!document.getElementById('lan-host').classList.contains('hidden') ||
                        !document.getElementById('lan-join').classList.contains('hidden')) {
                        this.cancelLan();
                    } else {
                        this.showMenu();
                    }
    ...
    initUI() {
        ...
        document.getElementById('btn-lan-join').addEventListener('click', () => {
            this.audioManager.playButton();
            this.showLanJoinForm();
        });
        document.getElementById('btn-lan-cancel').addEventListener('click', () => {
            this.audioManager.playButton();
            this.cancelLan();
        });
        document.getElementById('btn-lan-connect').addEventListener('click', () => {
            this.audioManager.playButton();
            this.connectLanJoin();
        });
        document.getElementById('btn-lan-join-back').addEventListener('click', () => {
            this.audioManager.playButton();
            this.showMenu();
        });
    ...
    showMenu() {
        ...
        document.getElementById('map-editor').classList.add('hidden');
        document.getElementById('pause-menu').classList.add('hidden');
        document.getElementById('lan-host').classList.add('hidden');
        document.getElementById('lan-join').classList.add('hidden');
    ...
    startGame(mode) {
        ...
        let p1Skill = this.selectedSkills.p1 || (mode === 'LAN' ? 'random' : 'none');
        if (p1Skill === 'random') {
        ...
        if (mode === 'PVP' || mode === 'LAN') {
            ...
            let p2Skill = this.selectedSkills.p2 || (mode === 'LAN' ? 'random' : 'none');
            if (p2Skill === 'random') {
    ...
    startLanHost() {
        ...
        this.networkManager.onError = (msg) => {
            game.showNotification('连接错误: ' + msg, 'error');
            game.showMenu();
        };

        this.networkManager.onRoomCreated = (roomId) => {
            document.getElementById('lan-room-code').textContent = roomId;
            document.getElementById('lan-host').classList.remove('hidden');
            game.showNotification('房间已创建，代码: ' + roomId, 'success', 5000);
        };

        this.networkManager.onGameStart = (playerIndex, initialState) => {
            game.playerIndex = playerIndex;
            game.isNetworkMode = true;
            document.getElementById('lan-host').classList.add('hidden');
            game.selectedSkills = { p1: null, p2: null };
            game.currentMode = 'LAN';
            game.startGame('LAN');
            game.applyServerState(initialState);
        };

        this.networkManager.onStateUpdate = (state) => {
            if (game.gameState === 'PLAYING') {
                game.applyServerState(state);
            }
        };

        this.networkManager.onPlayerDisconnected = () => {
            game.handleLanDisconnect();
        };

        const serverIp = 'localhost';
        const serverPort = '3000';

        this.networkManager.connect(serverIp, serverPort)
            .then(() => {
                game.networkManager.createRoom();
            })
            .catch(() => {
                game.showNotification('无法连接到服务器 ' + serverIp + ':' + serverPort + '，请确保服务器已启动', 'error');
    ...
    showLanJoinForm() {
        document.getElementById('main-menu').classList.add('hidden');
        document.getElementById('lan-join').classList.remove('hidden');
        document.getElementById('lan-join-status').classList.add('hidden');
    }

    connectLanJoin() {
        const serverIp = document.getElementById('lan-server-ip').value.trim();
        const serverPort = document.getElementById('lan-server-port').value.trim();
        const roomId = document.getElementById('lan-room-input').value.trim().toUpperCase();
        const statusEl = document.getElementById('lan-join-status');

        if (!serverIp || !serverPort || !roomId) {
        ...
        if (roomId.length !== 4) {
            statusEl.textContent = '房间代码必须为4位字符';
            statusEl.classList.remove('hidden');
            return;
        }
        ...
        this.networkManager.connect(serverIp, serverPort)
            .then(() => {
                statusEl.textContent = '加入房间 ' + roomId + '...';
                statusEl.style.color = '#aaa';
                game.networkManager.joinRoom(roomId);
            })
            .catch(() => {
                statusEl.textContent = '无法连接到 ' + serverIp + ':' + serverPort;
                statusEl.style.color = '#ff4444';
                statusEl.classList.remove('hidden');
    ...
    handleLanDisconnect() {
        if (this.gameState === 'PLAYING') {
            ...
            document.getElementById('winner-text').innerText = '对方已断开连接';
    ...
    update(deltaTime) {
        ...
        if (this.isNetworkMode && this.networkManager && this.networkManager.connected) {
            const localTank = this.tanks[this.playerIndex];
            this.networkManager.sendInput(localTank, this.input);
            this.updateHUD();
            return;
        }
        ...
        if (this.tanks.some(t => t.health <= 0)) {
            ...
            if (winnerIndex === 0) {
                winner = 'Player 1';
            } else if (this.currentMode === 'PVP' || this.currentMode === 'LAN') {
                winner = 'Player 2';
            } else {
                winner = 'Computer';
            }
    ...
    applyServerState(state) {
        if (state.tanks) {
            state.tanks.forEach((st, i) => {
                ...
                tank.x = st.x;
                tank.y = st.y;
                tank.rotation = st.rotation;
                tank.health = st.health;
                tank.selectedSkill = st.selectedSkill;
                tank.skillCooldown = st.skillCooldown;
                tank.skillActive = st.skillActive;
                tank.skillDuration = st.skillDuration;
                tank.isShielded = st.isShielded;
                tank.damageMultiplier = st.damageMultiplier;
                tank.canBulletBounce = st.canBulletBounce;
                tank.canBulletThroughWall = st.canBulletThroughWall;
                tank.canScatter = st.canScatter;
                tank.isInvisible = st.isInvisible;
                tank.isLaserActive = st.isLaserActive;
                tank.laserDisplayTime = st.laserDisplayTime;
                tank.laserStartX = st.laserStartX;
                tank.laserStartY = st.laserStartY;
                tank.laserEndX = st.laserEndX;
                tank.laserEndY = st.laserEndY;
                tank.energyShields = st.energyShields || [];
                tank.clones = (st.clones || []).map(c => ({
                    ...c,
                    isInvisible: c.isInvisible || false
                }));
            });
        }
        ...
        if (state.bullets) {
            this.bullets = state.bullets.map(b => ({
                ...
                radius: b.radius || 3, color: b.color,
                canBounce: b.canBounce || false,
                canThroughWall: b.canThroughWall || false,
                damageMultiplier: b.damageMultiplier || 1,
                isInvisible: b.isInvisible || false,
                angle: b.angle || 0,
                ...
                draw(ctx) {
                    ...
                    if (this.canBounce) ctx.fillStyle = '#ffaa00';
                    else if (this.canThroughWall) ctx.fillStyle = '#aa00ff';
                    ...
                    if (this.canBounce) {
                        ...
                        ctx.lineWidth = 1;
                        const trail = 15;
                        const px = this.x - Math.cos(this.angle) * trail;
                        const py = this.y - Math.sin(this.angle) * trail;
                        ctx.beginPath();
                        ctx.moveTo(px, py);
        ...
        if (state.gameOver && this.gameState === 'PLAYING') {
